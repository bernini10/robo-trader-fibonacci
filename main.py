# main.py (Versão 10.2 - Filtro de Chance Mínima 85%)
...
def obter_top_pares_por_liquidez(client, num_pares, pares_a_ignorar):
    ...
    try:
        ...
        METRICAS_PARES_MONITORADOS.info({'pares': ', '.join(lista_pares)})
        return lista_pares
    except Exception as e:
        log(f"Erro ao obter pares: {e}")
        METRICAS_PARES_MONITORADOS.info({'pares': 'Erro ao obter lista'})
        return []
...
async def main():
    ...
    # PARÂMETRO ALTERADO AQUI
    params = {'num_pares': 50, 'timeframes': ['15m', '1h', '4h'], 'periodo_zigzag': 15, 'pares_a_ignorar': ['USDCUSDT', 'FDUSDUSDT', 'TUSDUSDT', 'USDPUSDT', 'BUSDUSDT'], 'stop_loss_buffer': 0.01, 'chance_minima': 85, 'intervalo_minutos': 5}
    ...
    try:
        ...
        sys.exit(1)
    await enviar_alerta_telegram(bot, TELEGRAM_CHAT_ID, "✅ *Robô Analisador Fibonacci + Elliott Iniciado*\nEstou monitorando o mercado para você.")
    try:
        while True:
            try:
                ...
                if num_oportunidades > 0:
                    ...
                    for _, row in novas_oportunidades.iterrows():
                        ...
                        if alerta_id not in alertas_enviados:
                            ...
                            METRICAS_INFO_ALERTAS.info({
                                'par': row['par'],
                                'timeframe': row['timeframe'],
                                'tipo': row['tipo'],
                                'chance': f"{row['chance_%']:.0f}%",
                                'entrada': formatar_preco(preco_entrada),
                                'sl': f"{formatar_preco(row['sl'])} (-{sl_percent:.2f}%)",
                                'tp1': f"{formatar_preco(row['tp1'])} (+{tp1_percent:.2f}%)",
                                'tp2': f"{formatar_preco(row['tp2'])} (+{tp2_percent:.2f}%)",
                                'contexto': row['contexto_elliott']
                            })
        ...
        await enviar_alerta_telegram(bot, TELEGRAM_CHAT_ID, "🛑 *Robô Analisador Fibonacci + Elliott Parado*\nO monitoramento foi encerrado.")
